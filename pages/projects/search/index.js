        ...
        <Table variant="simple">
          ...
          <Thead>
            <Tr>
              ...
              <Th fontSize={"1rem"} isNumeric>New Deaths</Th>
              <Th fontSize={"1rem"} isNumeric>Total Confirmed</Th>
              <Th fontSize={"1rem"} isNumeric>Total Deaths</Th>
            </Tr>
          </Thead>
          <Tbody>
          {data.filter((i)=>i.Country.toLowerCase().includes(query)).map(i=>{
            return (<Tr key={i.id}>
              <Td>{i.Country}</Td>
...
const Search = () => {
  ...
  return (
    <Container maxW="container.xl" marginTop={'50px'} minHeight="85vh">
      ...
      <h2 style={{textAlign:"center",margin:"30px 0", fontWeight:600,fontSize:"32px"}}>Corona tracker with search filters</h2>
